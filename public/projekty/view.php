 ...
 * - Sekcja "Wydatki projektu" zamiast "Wydatki Pracowników"
 * - Wizualne oddzielenie modułów (kolorowe border-top)
 * - Kolejność: Finansowanie → Faktury sprzedażowe → Faktury kosztowe → Wydatki projektu → Robocizna
 * 
...
// Sprzedaż razem (faktury + PZF)
$total_sales_combined = $total_invoiced + $total_pzf_net;

// Kasa razem = zapłacone FV (pro-rata na projekt) + opłacone PZF (alokacja na projekt)
$total_cash_in = $total_received + $total_pzf_paid;
...
// =====================================================
try {
    $company_expense_summary_union = '';
    if ($expense_worker_filter === '') {
        $company_expense_summary_union = "
            UNION ALL
            SELECT fi.id as item_id, fi.amount_net as amount
    ...
    $expenses_sql = "
        ...
        FROM (
            SELECT we.id as item_id, we.amount as amount
...
// Expenses list
try {
    $company_expense_list_union = '';
    if ($expense_worker_filter === '') {
        $company_expense_list_union = "
            UNION ALL
            SELECT
                'company' as expense_source,
                fi.id,
                fi.issue_date as date,
                fi.title as description,
            ...
            WHERE fi.project_id = :project_id
              AND fi.item_type = 'FIXED_COST'
              AND fi.status IN ('pending', 'approved')
              {$company_expense_date_condition}
        ";
    }
...
// Zysk kasowy (na podstawie faktycznie otrzymanej kasy: wpłaty FV + opłacone PZF)
$profit_cash = $total_cash_in - $total_costs;
$margin_cash = ($total_cash_in > 0) ? ($profit_cash / $total_cash_in * 100) : 0;
...
// =====================================================
// TREND 12-MIESIĘCZNY (dla mini-wykresu projektu)
// =====================================================
$trend_labels = [];
$trend_revenue = [];
$trend_cash    = [];
$trend_costs   = [];
$monthNamesPl = ['', 'Sty', 'Lut', 'Mar', 'Kwi', 'Maj', 'Cze', 'Lip', 'Sie', 'Wrz', 'Paź', 'Lis', 'Gru'];
for ($i = 11; $i >= 0; $i--) {
    $mStart = date('Y-m-01', strtotime("first day of -$i month"));
    $mEnd   = date('Y-m-t',  strtotime($mStart));
    $trend_labels[]  = $monthNamesPl[(int)date('n', strtotime($mStart))] . ' ' . date('y', strtotime($mStart));
    $trend_revenue[] = round(analyticsRevenueTotal($pdo, $mStart, $mEnd, (int)$project_id), 2);
    $trend_cash[]    = round(analyticsCashInTotal($pdo, $mStart, $mEnd, (int)$project_id), 2);
    $costsM          = analyticsCosts($pdo, $mStart, $mEnd, (int)$project_id);
    $trend_costs[]   = round((float)$costsM['total'], 2);
}

// =====================================================
// ALERTY KONTROLERA (dla projektu)
// =====================================================
$project_alerts = [];

// 1) Marża kasowa < 0 przy > 50% zafakturowania
if ($total_revenue > 0 && ($total_invoiced / $total_revenue) > 0.5 && $profit_cash < 0) {
    $project_alerts[] = [
        'level' => 'warning',
        'icon'  => '⚠',
        'title' => 'Ujemna marża kasowa',
        'body'  => 'Projekt jest w ' . number_format($total_invoiced / $total_revenue * 100, 0)
                 . '% zafakturowany, ale po kasie jest na minusie (' . formatMoney($profit_cash) . '). Sprawdź, czy wszystkie wpłaty są zaksięgowane.',
    ];
}

// 2) Koszty przekraczają wartość umowy
if ($total_revenue > 0 && $total_costs > $total_revenue) {
    $over = $total_costs - $total_revenue;
    $project_alerts[] = [
        'level' => 'danger',
        'icon'  => '✖',
        'title' => 'Koszty przekraczają wartość umowy',
        'body'  => 'Łączne koszty (' . formatMoney($total_costs) . ') są wyższe niż wartość umowy (' . formatMoney($total_revenue) . ') o ' . formatMoney($over) . '. Projekt jest na minusie.',
    ];
}

// 3) Faktury FV przeterminowane > 14 dni
$overdue_fv = 0; $overdue_fv_amount = 0;
try {
    $stmt = $pdo->prepare("
        SELECT COUNT(*) AS cnt, COALESCE(SUM(amount_net - COALESCE(paid.paid_net, 0)), 0) AS amt
        FROM invoices_sale inv
        JOIN invoice_sale_allocations isa ON isa.invoice_id = inv.id
        LEFT JOIN (
            SELECT invoice_id, SUM(amount_net) AS paid_net FROM invoice_sale_payments GROUP BY invoice_id
        ) paid ON paid.invoice_id = inv.id
        WHERE isa.project_id = :pid
          AND inv.status = 'issued'
          AND inv.due_date IS NOT NULL
          AND inv.due_date < DATE_SUB(CURDATE(), INTERVAL 14 DAY)
    ");
    $stmt->execute(['pid' => $project_id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
    $overdue_fv        = (int)($row['cnt'] ?? 0);
    $overdue_fv_amount = (float)($row['amt'] ?? 0);
} catch (Throwable $e) {}
if ($overdue_fv > 0) {
    $project_alerts[] = [
        'level' => 'warning',
        'icon'  => '⚠',
        'title' => 'Przeterminowane faktury (> 14 dni)',
        'body'  => $overdue_fv . ' nieopłacone faktury sprzedażowe na łączną kwotę ' . formatMoney($overdue_fv_amount) . '. Rozważ monity.',
    ];
}

// 4) PZF po terminie (unpaid > 30 dni od wystawienia)
$overdue_pzf = 0; $overdue_pzf_amount = 0;
try {
    $stmt = $pdo->prepare("
        SELECT COUNT(*) AS cnt, COALESCE(SUM(a.amount_net), 0) AS amt
        FROM sales_noninvoice_allocations a
        JOIN sales_noninvoice_entries e ON e.id = a.entry_id
        WHERE a.project_id = :pid
          AND e.payment_status = 'unpaid'
          AND e.issue_date < DATE_SUB(CURDATE(), INTERVAL 30 DAY)
    ");
    $stmt->execute(['pid' => $project_id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
    $overdue_pzf        = (int)($row['cnt'] ?? 0);
    $overdue_pzf_amount = (float)($row['amt'] ?? 0);
} catch (Throwable $e) {}
if ($overdue_pzf > 0) {
    $project_alerts[] = [
        'level' => 'warning',
        'icon'  => '⚠',
        'title' => 'PZF bez wpływu (> 30 dni)',
        'body'  => $overdue_pzf . ' wpisów pozafakturowych bez statusu opłacony na łączną kwotę ' . formatMoney($overdue_pzf_amount) . '. Sprawdź, czy pieniądze nie wpłynęły.',
    ];
}

// 5) Brak alokacji na stage - poinformuj jeśli jest dużo kosztów bez przypisanego etapu (opcjonalnie później)

if (empty($project_alerts) && $total_costs > 0) {
    $project_alerts[] = [
        'level' => 'ok',
        'icon'  => '✓',
        'title' => 'Projekt bez alertów',
        'body'  => 'Żaden mechanizm kontrolny nie zgłasza uwag. Pilnujemy dalej.',
    ];
}
...
<head>
    ...
    <style>
        ...
        /* =================================================
         * ANALIZA FINANSOWA — zgodne ze styl.md (kpi-card 12px radius, akcent #667eea)
         * ================================================= */
        .fin-analysis-header {
        ...
        .fin-analysis-header h2 {
            font-size: 18px; font-weight: 700; color: #1f2937; letter-spacing: -0.2px;
        }
        ...
        /* Akcenty kolorem wartosci (zamiast paska na gorze) */
        .fin-kpi.sales .fin-kpi-value,
        .fin-kpi.cash  .fin-kpi-value     { color: #16a34a; }
        .fin-kpi.cash-due .fin-kpi-value  { color: #ea580c; }
        .fin-kpi.retention .fin-kpi-value { color: #b45309; }
        .fin-kpi.costs .fin-kpi-value     { color: #dc2626; }
        .fin-kpi.profit .fin-kpi-value    { color: #1f2937; }
        .fin-kpi.profit.negative .fin-kpi-value { color: #dc2626; }
        .fin-kpi.profit.positive .fin-kpi-value { color: #16a34a; }
        .fin-kpi.margin .fin-kpi-value    { color: #0891b2; }

        /* Tooltip bez ikony, natychmiastowy (jak w overview.php) */
        ...
        /* Alerty kontrolera — spojne z overview.php */
        .fin-alerts {
        ...
        .fin-trend-legend { display: flex; gap: 14px; font-size: 12px; color: #6b7280; }
        .fin-trend-legend span { display: inline-flex; align-items: center; gap: 6px; }
        .fin-trend-legend i {
            width: 10px; height: 10px; border-radius: 2px; display: inline-block;
        }
        .fin-trend-chart { height: 240px; position: relative; }
...
<body>
    ...
    <div class="container">
        ...
        <!-- ============================================= -->
        <!-- ANALIZA FINANSOWA — 4 grupy (Sprzedaż · Kasa · Koszty · Zysk) -->
        <!-- ============================================= -->
        ...
        <?php /* ---------- GRUPA 1: SPRZEDAŻ ---------- */ ?>
        <div class="fin-group">
            <div class="fin-group-title">Sprzedaż</div>
            <div class="fin-group-grid">
                <div class="fin-kpi sales" data-tooltip="Wartość wszystkich umów z inwestorem razem z aneksami. To docelowy przychód projektu — baza do liczenia marży umownej.">
                    <div class="fin-kpi-label">Wartość umowy</div>
                    <div class="fin-kpi-value" style="color:#16a34a;"><?php echo formatMoney($total_revenue); ?></div>
                    <div class="fin-kpi-sub">Umowy + aneksy</div>
                </div>
                <div class="fin-kpi sales-fv" data-tooltip="Suma faktur sprzedażowych zaalokowanych do projektu (netto). Data wystawienia faktury. Pomija wersje robocze i anulowane.">
                    <div class="fin-kpi-label">Zafakturowano (FV)</div>
                    <div class="fin-kpi-value" style="color:#2563eb;"><?php echo formatMoney($total_invoiced); ?></div>
                    <div class="fin-kpi-sub">
                        <?php echo count($sales_invoices_list); ?> faktur
                        <?php if ($total_revenue > 0): ?>
                            · <?php echo number_format($total_invoiced / $total_revenue * 100, 0); ?>% umowy
                        <?php endif; ?>
                    </div>
                </div>
                <div class="fin-kpi sales-pzf" data-tooltip="Przychody zaksięgowane poza fakturami (PZF) — np. przedpłaty, zaliczki gotówkowe, transakcje bez faktury. Netto.">
                    <div class="fin-kpi-label">Pozafakturowo (PZF)</div>
                    <div class="fin-kpi-value" style="color:#7c3aed;"><?php echo formatMoney($total_pzf_net); ?></div>
                    <div class="fin-kpi-sub"><?php echo count($pzf_entries_list); ?> wpisów</div>
                </div>
                <div class="fin-kpi" data-tooltip="Zafakturowano (FV) + Pozafakturowo (PZF). Pełna sprzedaż projektu netto, niezależnie od tego czy pieniądze już wpłynęły.">
                    <div class="fin-kpi-label">Sprzedaż razem</div>
                    <div class="fin-kpi-value" style="color:#0891b2;"><?php echo formatMoney($total_sales_combined); ?></div>
                    <div class="fin-kpi-sub">FV + PZF łącznie</div>
                </div>
            </div>
        </div>
        ...
        <div class="fin-group">
            ...
            <div class="fin-group-grid">
                <div class="fin-kpi cash" data-tooltip="Pieniądze, które faktycznie wpłynęły na konto w kontekście tego projektu: wpłaty od klientów na FV (pro-rata według alokacji) + PZF o statusie opłacony.">
                    <div class="fin-kpi-label">Wpłynęło na konto</div>
                    <div class="fin-kpi-value" style="color:#059669;"><?php echo formatMoney($total_cash_in); ?></div>
                    <div class="fin-kpi-sub">
                        <?php if ($total_pzf_paid > 0 && $total_received > 0): ?>
                            FV: <?php echo formatMoney($total_received); ?> · PZF: <?php echo formatMoney($total_pzf_paid); ?>
                        <?php elseif ($total_pzf_paid > 0): ?>
                            PZF: <?php echo formatMoney($total_pzf_paid); ?>
                        <?php elseif ($total_received > 0): ?>
                            FV: <?php echo formatMoney($total_received); ?>
                        <?php elseif ($total_sales_combined > 0): ?>
                            <?php echo number_format($total_cash_in / $total_sales_combined * 100, 0); ?>% sprzedaży
                        <?php else: ?>—<?php endif; ?>
                    </div>
                </div>
                <div class="fin-kpi cash-due" data-tooltip="Wystawione, ale jeszcze nieopłacone faktury sprzedażowe (netto). To pieniądze, których klient jeszcze nie przelał — do pilnowania.">
                    <div class="fin-kpi-label">Do zapłaty (FV)</div>
                    <div class="fin-kpi-value" style="color:<?php echo $to_pay > 0 ? '#dc2626' : '#6b7280'; ?>;"><?php echo formatMoney($to_pay); ?></div>
                    <div class="fin-kpi-sub">
                        <?php echo $to_pay > 0 ? 'Nieopłacone faktury' : 'Wszystko opłacone'; ?>
                    </div>
                </div>
                <?php if ($total_retention_active > 0 || $total_retention_settled > 0): ?>
                <div class="fin-kpi retention" data-tooltip="Kwota zatrzymana przez inwestora jako zabezpieczenie (retencja/kaucja). Aktywna = jeszcze niezwrócona. Rozliczona = już oddana przez inwestora.">
                    <div class="fin-kpi-label">Retencja</div>
                    <div class="fin-kpi-value" style="color:#b45309;"><?php echo formatMoney($total_retention_active); ?></div>
                    <div class="fin-kpi-sub">
                        Aktywna (u klienta)
        ...
        <?php /* ---------- GRUPA 3: KOSZTY ---------- */ ?>
        <div class="fin-group">
            <div class="fin-group-title">Koszty projektu <?php if ($has_date_filter): ?><span style="color:#94a3b8;font-weight:normal;text-transform:none;letter-spacing:0;">· filtr: <?php echo $date_from ? formatDate($date_from) : '…'; ?> – <?php echo $date_to ? formatDate($date_to) : '…'; ?></span><?php endif; ?></div>
            <div class="fin-group-grid">
                <div class="fin-kpi costs" data-tooltip="Łączne koszty projektu netto: faktury kosztowe + wydatki projektu + robocizna (zatwierdzone work-logi). Uwzględnia filtr daty.">
                    <div class="fin-kpi-label">Koszty razem</div>
                    <div class="fin-kpi-value kpi-negative"><?php echo formatMoney($total_costs); ?></div>
                    <div class="fin-kpi-sub">Faktury + wydatki + robocizna</div>
                </div>
                <div class="fin-kpi" data-tooltip="Zatwierdzone faktury kosztowe zaalokowane do projektu (netto). Klik w sekcję Faktury kosztowe niżej — zobaczysz listę.">
                    <div class="fin-kpi-label">Faktury kosztowe</div>
                    <div class="fin-kpi-value" style="color:#1d4ed8;"><?php echo formatMoney($invoices_materials); ?></div>
                    <div class="fin-kpi-sub"><?php echo (int)($invoices_summary_data['invoices_count'] ?? 0); ?> dokumentów</div>
                </div>
                <div class="fin-kpi" data-tooltip="Zatwierdzone wydatki firmowe i pracownicze zaalokowane do projektu. Netto.">
                    <div class="fin-kpi-label">Wydatki projektu</div>
                    <div class="fin-kpi-value" style="color:#7c3aed;"><?php echo formatMoney($expenses_cash); ?></div>
                    <div class="fin-kpi-sub"><?php echo (int)($expenses_summary['expenses_count'] ?? 0); ?> pozycji</div>
                </div>
                <div class="fin-kpi" data-tooltip="Koszt robocizny: zatwierdzone godziny pracowników na tym projekcie × stawka. Uwzględnia godziny normalne, nadliczbowe, nocne, weekendowe.">
                    <div class="fin-kpi-label">Robocizna</div>
                    <div class="fin-kpi-value" style="color:#c026d3;"><?php echo formatMoney($labor_cost); ?></div>
                    <div class="fin-kpi-sub"><?php echo number_format(($labor_summary['total_hours'] ?? 0) + ($labor_summary['total_overtime'] ?? 0), 1, ',', ' '); ?> h</div>
                </div>
            </div>
        </div>
        ...
        <div class="fin-group">
            ...
            <div class="fin-group-grid">
                <div class="fin-kpi profit" data-tooltip="Zysk dokumentowy: zafakturowana sprzedaż (FV) − koszty projektu netto. To zysk teoretyczny — wg dokumentów, niezależnie od tego czy klient zapłacił.">
                    <div class="fin-kpi-label">Zysk (dokument)</div>
                    <div class="fin-kpi-value <?php echo $profit_class; ?>"><?php echo formatMoney($profit_real); ?></div>
                    <div class="fin-kpi-sub">Zafakturowano − koszty</div>
                </div>
                <div class="fin-kpi profit" data-tooltip="Zysk kasowy: pieniądze faktycznie otrzymane (FV + PZF) − koszty netto. Najbardziej prawdziwy obraz zarobku na tym projekcie.">
                    <div class="fin-kpi-label">Zysk (kasa)</div>
                    <div class="fin-kpi-value <?php echo $profit_cash_class; ?>"><?php echo formatMoney($profit_cash); ?></div>
                    <div class="fin-kpi-sub">Wpłynęło (FV+PZF) − koszty</div>
                </div>
                <div class="fin-kpi margin" data-tooltip="Marża dokumentowa: zysk dokument / zafakturowana sprzedaż. Pod spodem marża umowna: zysk względem wartości umowy (docelowy poziom zarobku na końcu projektu).">
                    <div class="fin-kpi-label">Marża dokumentowa</div>
                    <div class="fin-kpi-value <?php echo $margin_class; ?>"><?php echo number_format($margin_real, 1); ?>%</div>
                    <div class="fin-kpi-sub">Umowna: <?php echo number_format($margin_contract, 1); ?>%</div>
                </div>
                <?php
                    $margin_cash_val = ($total_cash_in > 0) ? ($profit_cash / $total_cash_in * 100) : null;
                    $margin_cash_cls = $margin_cash_val === null ? 'kpi-neutral'
                        : ($margin_cash_val > 20 ? 'kpi-positive' : ($margin_cash_val > 0 ? 'kpi-warning' : 'kpi-negative'));
                ?>
                <div class="fin-kpi" data-tooltip="Marża kasowa: zysk kasowy / wpłynęło na konto. Pokazuje, ile procent z tego co naprawdę przyszło na konto zostaje firmie po opłaceniu kosztów.">
                    <div class="fin-kpi-label">Marża kasowa</div>
                    <div class="fin-kpi-value <?php echo $margin_cash_cls; ?>">
                        <?php echo $margin_cash_val !== null ? number_format($margin_cash_val, 1) . '%' : '—'; ?>
                    </div>
                    <div class="fin-kpi-sub">
                        <?php echo $margin_cash_val !== null ? 'Realnie z gotówki' : 'Brak wpływów'; ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="fin-trend-wrap">
            <div class="fin-trend-head">
                ...
                <div class="fin-trend-legend">
                    ...
                    <span><i style="background:#059669;"></i> Wpłynęło</span>
                    <span><i style="background:#dc2626;"></i> Koszty</span>
                </div>
            </div>
            <div class="fin-trend-chart"><canvas id="finTrendChart"></canvas></div>
        </div>

        <?php /* ---------- ALERTY KONTROLERA ---------- */ ?>
        <?php if (!empty($project_alerts)): ?>
        <div class="fin-analysis-header" style="margin-top:10px;">
            <h2 style="font-size:15px;">Uwagi kontrolera</h2>
            <div class="fin-legend">Automatyczne alerty, które warto sprawdzić</div>
        </div>
        <div class="fin-alerts">
            <?php foreach ($project_alerts as $al): ?>
                <div class="fin-alert <?php echo e($al['level']); ?>">
                    <div class="fin-alert-icon"><?php echo e($al['icon']); ?></div>
                    <div class="fin-alert-body">
                        <div class="fin-alert-title"><?php echo e($al['title']); ?></div>
                        <div class="fin-alert-text"><?php echo e($al['body']); ?></div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <!-- Guardrail: oddzielna tabela, nie dotyka FV    -->
        <!-- ============================================= -->
        ...
        <div class="module-section" id="przychody-pzf">
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:0;">
                    ...
                    <p class="section-note" style="color:#7c3aed;">Przychody pozafakturowe przypisane do projektu. Nieopłacone liczą się do "Do wpływu".</p>
                </div>
                <?php else: ?>
                <div class="card-body">
        ...
        <!-- ============================================= -->
        <!-- MODUŁ 4: WYDATKI PROJEKTU                     -->
        <!-- ============================================= -->
        <div class="module-section" id="wydatki">
            <div class="module-accent module-accent-red"></div>
            <div class="card">
                <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
                    <h3>Wydatki projektu (<?php echo $expenses_summary['expenses_count'] ?? 0; ?>)</h3>
                    <?php if ($isAdminUser): ?>
                        <a href="<?php echo url('finanse.wydatki.create', [
                            'project_id' => $project_id,
                            'return_url' => url('projekty.view', ['id' => $project_id, 'success' => 'expense_added']) . '#wydatki'
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if (($exp['expense_source'] ?? '') !== 'company' && (!empty($exp['paid_by_employee']) || strpos($exp['description'], '[PAID_BY_EMPLOYEE]') !== false)): ?>
    ...
    <script>
        ...
        new Chart(ctx, {
            type: 'pie',
            data: {
                labels: ['Faktury kosztowe', 'Wydatki projektu', 'Robocizna'],
                datasets: [{
        ...
        // =====================================================
        // MINI TREND 12M — projekt (sprzedaż · kasa · koszty)
        // =====================================================
        (function () {
            ...
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: <?php echo json_encode($trend_labels, JSON_UNESCAPED_UNICODE); ?>,
                    datasets: [
                        {
                            label: 'Sprzedaż (FV+PZF)',
                            data: <?php echo json_encode($trend_revenue); ?>,
                            borderColor: '#0891b2',
                            backgroundColor: 'rgba(8, 145, 178, 0.08)',
                            fill: true, tension: 0.35, borderWidth: 2,
                            pointRadius: 3, pointBackgroundColor: '#0891b2'
                        },
                        {
                            label: 'Wpłynęło',
                            data: <?php echo json_encode($trend_cash); ?>,
                            borderColor: '#059669',
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: function (ctx) {
                                    const v = ctx.raw || 0;
                                    return ctx.dataset.label + ': ' + Number(v).toLocaleString('pl-PL', { style: 'currency', currency: 'PLN' });
                                }
                            }
                        }
                    },
                    scales: {
                        y: {
                            ...
                            ticks: {
                                callback: function (v) {
                                    if (Math.abs(v) >= 1000) return (v / 1000).toFixed(0) + ' tys.';
                                    return v;
                                }
                            },
